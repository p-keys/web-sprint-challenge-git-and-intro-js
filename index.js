...
function getArtistByIndex(a, num) {
  return(`the artist at index ${num} is ${a[num].name}`); 
}
...
function listOfNames(a) {
  let copy = []; 
  for(let key = 0; key < artists.length; key++)
  {
    copy[key] = a[key].name; 
  }
  return(copy); 
...
function addArtist(a, name, years, genre, nationality, bio) {
  const createdobj = 
  {
    name : name, 
    years : years,
...
function artistByCountry(a, nationality){
  let names = []; 
  for(let i = 0; i < artists.length; i++)
  {
